...
### v2.10.7 (2025-09-15)
- 🎯 **버튼/SB/BB 선택 로직 개선**
  - 전체 플레이어 리스트에서 버튼 선택 가능
  - 게임 참여 여부와 관계없이 모든 테이블 플레이어 표시
  - SB/BB 자동 계산이 전체 플레이어 기준으로 동작
  - 참여중(🟢)/대기중(⚫) 상태 표시 추가

### v2.10.6 (2025-09-15)
- 🐛 **버튼 드롭다운 작동 버그 수정**
  - seatMap 업데이트 로직 추가
  - togglePlayerInHand에서 seatMap 동기화
  - 버튼 위치 선택 기능 정상 작동
- 🔧 **버전 중앙 관리 시스템 구현**
  - APP_VERSION 상수를 통한 중앙화된 버전 관리
  - updateAllVersionDisplays() 함수로 자동 업데이트

### v2.10.5 (2025-09-15)
- 🐛 **버튼 위치 중복 입력 버그 수정**
  - 이벤트 리스너 중복 등록 방지
  - cloneNode를 사용하여 기존 리스너 제거
  - updatePositionIndicators 중복 호출 제거

### v2.10.4 (2025-09-15)
- 🌐 **버튼 설정 UI 최적화**
  - 1x11 그리드로 공간 효율성 개선
  - 드롭다운 작동 문제 해결
  - 모바일 화면에서 더 많은 콘텐츠 표시
  - SB/BB 표시 영역 유지
