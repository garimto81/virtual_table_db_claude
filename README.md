...
## 최신 업데이트 (v9.3.4 - 2025-01-09)

### 🔧 **핫픽스: Apps Script async/await 구문 오류 수정 (v9.3.4)**

**✅ 완료된 주요 수정사항:**

1. **Apps Script async/await 구문 오류 수정** ✓
   - `handleSheetUpdate()` 함수를 `async`로 선언 (라인 74)
   - `handleHandUpdate()` 함수를 `async`로 선언하고 `await` 추가 (라인 192, 240)
   - "SyntaxError: await is only valid in async functions" 오류 해결

### 🔧 **이전 핫픽스: Apps Script 호환성 문제 해결 (v9.3.3)**

**✅ 완료된 주요 수정사항:**

1. **Apps Script 호환성 문제 해결** ✓
   - 배포된 Apps Script에서 `updateHand` 액션 미지원으로 인한 "지원되지 않는 액션" 오류 수정
   - 기존 `updateSheet` 액션을 사용하도록 코드 수정 (즉시 해결)
...
3. **Apps Script 배포 가이드 추가** 📋
   - 로컬 코드 수정 후 실제 배포가 필요함을 명확히 문서화
   - `apps-script/README.md`에 배포 체크리스트 및 트러블슈팅 추가
