...
- 📊 **승자 정보 Google Sheets 저장**
  - Index 시트 R열에 승자 정보 추가
  - Apps Script 헤더 및 데이터 구조 업데이트
  - 복수 승자 지원 (쉼표로 구분)
...
### v2.10.9 (2025-09-15)
- 🐛 **팟 계산 로직 버그 수정**
  - 중간 베팅 금액이 누적되어 잘못 계산되던 문제 해결
  - 각 스트리트별 최종 베팅 금액만 계산하도록 수정
  - 예: 25K 벳→48K 3벳→48K 콜 = 각자 48K씩, 총 96K 추가 (기존 잘못된 계산: 121K)

### v2.10.8 (2025-09-15)
- 🐛 **키패드 중복 입력 버그 수정**
  - 6을 한번 누르면 66이 입력되던 문제 해결
  - 중복 이벤트 리스너 제거 및 전역 키패드 이벤트로 통합
  - openChipInput, SB/BB 버튼 키패드 로직 개선
  - 키패드 목적별 분기 처리 추가
