...
### v3.4.3 (2025-09-18) - 로컬 중복 검출 시 실제 삭제 처리
- 🎯 **중복 삭제 로직 수정**: 로컬에서 중복 발견 시 스킵하지 않고 실제 구글 시트에서 삭제
- 🔧 **CSV 파싱 로직 수정**: buildTypeFromCsv에서 잘못된 중복 필터링 제거
- 📊 **정확한 중복 검출**: table+name+seat 기준으로만 진짜 중복 체크하도록 수정
- 🔍 **데이터 동기화**: 구글 시트 17명 vs 로컬 10명 불일치 문제 해결
- 🔒 **UI 블록킹**: 중복 제거 작업 중 충돌 방지를 위한 전체 UI 잠금
